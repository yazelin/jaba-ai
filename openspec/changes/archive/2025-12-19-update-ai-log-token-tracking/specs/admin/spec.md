...
系統 SHALL 提供 AI 對話日誌檢視功能，供超級管理員分析 AI 行為、token 使用量和調整 prompt。
...
- **THEN** 系統顯示完整詳情：輸入的完整 prompt context、AI 原始回應（包含思考過程）、解析出的 message 和 actions、執行時間、輸入 token 數量、輸出 token 數量
